...
class CPatient:
    ...
    def res(self, cur, first, second, third, fourth, day):
        fm = np.array([
            self.tx,
            day,
    ...
    def getFeatureMaps(self, is_e=False):
        ...
        if is_e:
            ...
            yield self.res(self.nullobs, first, second, third, fourth, TEST_DAY)
        else:
            for i, obs in enumerate(self.observations[1:]):
                ...
                yield self.res(cur, first, second, third, fourth, cur.day)
...
def load_data_c(filenames, e_pids=None):
    ...
    for filename in filenames:
        with open(filename) as cv:
            ...
            vals = [row.split(',') for row in cv]
            nrows = len(vals)
            patients = {(data[2], data[6]) for idx, data in enumerate(vals) if idx != 0}
            patientMap = {id: CPatient(id, tx) for idx, (id, tx) in enumerate(patients)}
